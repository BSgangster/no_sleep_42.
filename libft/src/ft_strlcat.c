...
size_t ft_strlcat(char *dest, const char *src,size_t size)
{
    ...
    while(dest[i] && len < size)//loop through dest string until we reach the end.
    {
        len++;
    }
    ...
    while(src[len - i] && len + i < size)//loop until we get to the end of the source loop, or until we reach the size we wanna copy.
    {
        dest[len] = src[len - i];
        len++;
    }
